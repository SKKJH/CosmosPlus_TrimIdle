...
#define AVAILABLE_DATA_BUFFER_ENTRY_COUNT				(16 * USER_DIES) //512
#define AVAILABLE_TEMPORARY_DATA_BUFFER_ENTRY_COUNT		(USER_DIES)

#define DELAY_BUF_NONE 0x3fff
...
//delay Buf ~
typedef struct _DELAY_BUF_ENTRY {
	unsigned int logicalSliceAddr : 28;
	unsigned int blk0 : 1;
	unsigned int blk1 : 1;
	unsigned int blk2 : 1;
...
typedef struct _DELAY_BUF_MAP{
	DELAY_BUF_ENTRY delayWriteBuf[16384]; //SLICES_PER_SSD / 1024
} DELAY_BUF_MAP, *P_DELAY_BUF_MAP;

typedef struct _DELAY_BUF_HASH_ENTRY{
	unsigned int headEntry : 16;
	unsigned int tailEntry : 16;
} DELAY_BUF_HASH_ENTRY, *P_DELAY_BUF_HASH_ENTRY;

typedef struct _DELAY_BUF_HASH_TABLE{
	DELAY_BUF_HASH_ENTRY delayBufHash[AVAILABLE_DATA_BUFFER_ENTRY_COUNT];
} DELAY_BUF_HASH_TABLE, *P_DELAY_BUF_HASH_TABLE;
...
extern P_DELAY_BUF_MAP delayBufMapPtr;
extern P_DELAY_BUF_HASH_TABLE delayBufHashTablePtr;
