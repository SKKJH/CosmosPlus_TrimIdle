...
void ReqTransSliceToLowLevel()
{
	...
	while(sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
	{
		...
		if((reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_WRITE) && (do_trim_flag == 1)) {
		    unsigned int delayEntry = CheckDelayBufHit(reqPoolPtr->reqPool[reqSlotTag].logicalSliceAddr);
			if(delayEntry == DATA_BUF_FAIL) {
				//xil_printf("delay hash miss\n");
				delayEntry = dw_cnt;
		...
		else if(reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_RxDMA)
			dataBufMapPtr->dataBuf[dataBufEntry].dirty = DATA_BUF_CLEAN;
		else
