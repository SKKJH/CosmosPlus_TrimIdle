...
P_DELAY_BUF_MAP delayBufMapPtr;
P_DELAY_BUF_HASH_TABLE delayBufHashTablePtr;
P_DSM_RANGE dmRangePtr;
...
void InitDataBuf()
{
	...
	delayBufMapPtr = (P_DELAY_BUF_MAP) DELAY_BUF_MAP_ADDR;
	delayBufHashTablePtr = (P_DELAY_BUF_HASH_TABLE) DELAY_BUF_HASH_TABLE_ADDR;
	...
	for(bufEntry = 0; bufEntry < AVAILABLE_DATA_BUFFER_ENTRY_COUNT; bufEntry++)
	{
		...
		// Delayed Write Buffer Entry~
		delayBufHashTablePtr->delayBufHash[bufEntry].headEntry = DATA_BUF_NONE;
		delayBufHashTablePtr->delayBufHash[bufEntry].tailEntry = DATA_BUF_NONE;
	}

	for(bufEntry = 0; bufEntry < 16384; bufEntry++)
	{
		delayBufMapPtr->delayWriteBuf[bufEntry].logicalSliceAddr = LSA_NONE;
		delayBufMapPtr->delayWriteBuf[bufEntry].t_time = 0xf;
		delayBufMapPtr->delayWriteBuf[bufEntry].blk0 = 0;
		delayBufMapPtr->delayWriteBuf[bufEntry].blk0 = 0;
		delayBufMapPtr->delayWriteBuf[bufEntry].blk0 = 0;
		delayBufMapPtr->delayWriteBuf[bufEntry].blk0 = 0;
		delayBufMapPtr->delayWriteBuf[bufEntry].hashPrevEntry = DELAY_BUF_NONE;
...
unsigned int CheckDelayBufHit(unsigned int LPN)
{
	...
	{
		if(delayBufMapPtr->delayWriteBuf[bufEntry].logicalSliceAddr == LPN) {
...
void PutToDelayBufHashList(unsigned int dw_cnt, unsigned int LPN)
{
	...
	if(delayBufHashTablePtr->delayBufHash[hashEntry].tailEntry != DATA_BUF_NONE)
	{
		...
		delayBufMapPtr->delayWriteBuf[delayBufHashTablePtr->delayBufHash[hashEntry].tailEntry].hashNextEntry = dw_cnt;
		delayBufHashTablePtr->delayBufHash[hashEntry].tailEntry = dw_cnt;
	}
	else
	{
		delayBufMapPtr->delayWriteBuf[dw_cnt].hashPrevEntry = DELAY_BUF_NONE;
		delayBufMapPtr->delayWriteBuf[dw_cnt].hashNextEntry = DELAY_BUF_NONE;
		delayBufHashTablePtr->delayBufHash[hashEntry].headEntry = dw_cnt;
		delayBufHashTablePtr->delayBufHash[hashEntry].tailEntry = dw_cnt;
	}
}
